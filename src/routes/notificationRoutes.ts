...
export default async function notificationRoutes(app: FastifyInstance) {
  ...
  app.post(
    "",
    {
      schema: {
        tags: ["Notifications"],
        body: zodToJsonSchema(createNotificationValidation('America/Sao_Paulo')),
        response: {
    ...
    async (req, res) => {
      ...
      let { data, error, success } = createNotificationValidation(user.timeZone || 'America/Sao_Paulo').safeParse(
        req.body
      );
      ...
      let notification = await notificationService.createFutureGoalNotification(
        data!,
        userId
      );
  ...
  app.put(
    ...
    async (req, res) => {
      ...
      let updatedNotification = await notificationService.updateNotification(
        req.user.id,
        body,
        params.id
      );
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      let { success, data, error } = getNotificationsValidation.safeParse(
        req.query
      );
  ...
  app.get(
    ...
    async (req, res) => {
      const userId = req.user.id;
      let { id } = req.params as { id: string };
      let notification = await notificationService.getNotification(
        userId,
        Number(id)
      );
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      await notificationService.updateNotificationMessageStatus(
        notificationId,
        query.accept ? "ACCEPTED" : "REJECTED"
      );

      query.accept
        ? res.status(200).send({ message: "Parcela criada" })
        : res.status(200).send({ message: "Parcela rejeitada" });
  ...
  app.put(
    ...
    async (req, res) => {
      ...
      await notificationService.changeNotificationActivation(
  ...
  app.get(
    "/messages",
    {
      schema: {
        ...
        response: {
          200: zodToJsonSchema(
            z.object({ notifications: z.array(confirmationsResponseSchema) })
          ),
        },
        security: [{ bearerAuth: [] }],
        summary:
          "Infomações referentes a notificação, como se fossem as notificações do insta. podem ser do tipo info ou confirm, c for confirm tem q ter a representação visual do X ou V para aceitar ou recusar a criação da parcelsa.",
      },
    },
    async (req, res) => {
      let userId = req.user.id;
      ...
      let notifications =
        await notificationService.GetNotificationMessages(userId);
