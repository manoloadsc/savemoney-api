...
export default async function privateUserRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      let data = {
        ...
        active: user.active,
        numberVerified: user.numberVerified_at!!,
        lang: user.lang,
        currency: user.currency,
        emailVerified: user.emailVerified_at!!,
        plan,
        completeInformation: user.completeInformation,
      };
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      let { messageId } = await whatssapService.activeAccountNumber(
        user.chat_id!,
        code.authCode
      );
