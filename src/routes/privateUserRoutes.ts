...
export default async function privateUserRoutes(app: FastifyInstance) {
  ...
  app.get("/me", {
    ...
    let plan: 'anual' | 'trimestral' | 'mensal' | string | undefined

    if(user.active){
      plan = await userService.getUserPlan(userId)
    }

    let data = { name: user.name, email: user.email, chat_id: user.chat_id, active: user.active, numberVerified: user.numberVerified_at!!, emailVerified: user.emailVerified_at!!, plan, currency: user.currency, lang: user.lang, completeInformation: user.completeInformation}
