...
import {
  authActivateValidation,
  createCheckoutWithEmail,
} from "validatation/auth.validation.js";
import {
  forgotEmailValidation,
  parseLogin,
  passwordValidation,
  userValidation,
  userCheckoutValidation,
} from "validatation/user.validation.js";
import { z } from "zod";
...
export default async function publicUserRoutes(app: FastifyInstance) {
  app.post(
    "/register",
    {
      schema: {
        ...
        description:
          "Deve retornar pra uma página informando o cliente que recebeu no email uma menssagem para a ativação da conta.",
      },
    },
    async (req, res) => {
  ...
  app.post(
    ...
    {
      schema: {
        body: zodToJsonSchema(userCheckoutValidation),
        response: {
          201: zodToJsonSchema(z.string().describe("email enviado")),
        },
        tags: ["User - Public"],
    ...
    async (req, res) => {
      let { data, error, success } = userCheckoutValidation.safeParse(req.body);

      if (!success) return res.status(400).send({ error: error! });

      if (!data) return res.status(400).send({ error: error });

      let defaultUser = await userService.createDefaultUser({
        email: data?.email,
        name: data?.name,
      ...
      await resendService.defaultUserCreated(
        ...
        defaultUser.randomPassowrd
      );

      res.status(201).send(`email enviado para ${defaultUser.user.email}`);
    }
  );
  ...
  app.get(
    "/validate",
    {
      schema: {
        ...
        description:
          "Rota que voce deve enviar para ativar a conta do usuario, vamos definir uma rota no frontend qe vai receber o token, após isso, vamos receber o token da query e enviar para o backend, oara ESSA rota",
      },
    },
    async (req, res) => {
  ...
  app.post(
    "/forgotPassword",
    {
      schema: {
        ...
        summary: "Forgot user",
        description:
          "Vai entriar para o email do cara um email de recuperação de senha.",
      },
    },
    async (req, res) => {
      ...
      let token = app.jwt.sign(
        { id: user!.id, email: user!.email, type: "RESET_PASSWORD", jti },
        { expiresIn: "2h" }
      );
      let tokenSaved = await authService.saveUserTokenForgotPassword(
        jti,
        email
      );
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      await userService.changePassowordByEmailSendedToken(
        infos,
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      let session = await stripeService.createCheckout(
        body.email,
        body.plan,
        body.phone,
        logged
