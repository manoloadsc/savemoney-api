...
import hotmartSevice from "./hotmart.sevice.js";
    ...
    cron.schedule(
      ...
      async () => {
        try {
          ...
          await hotmartSevice.runPlanExpiryJob();
      ...
      if (nextDay.getDate() === 27) {
        ...
        const promises = users.map(async (user) => {
          ...
          try {
            ...
            console.log(
              `Erro ao enviar notificação para o usuário: ${user.chat_id} ${user.name}`
            );
          }
        });
