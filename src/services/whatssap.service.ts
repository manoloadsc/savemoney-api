...
class WhatssapService {
  ...
  async sendUserNotFound(to: string) {
    const message = `¡Hola! 👋 Hemos identificado que aún no tienes un registro activo o tu suscripción no está al día.

Para seguir usando SaveMoneyy es necesario registrarte o reactivar tu suscripción.

👉 Haz clic aquí para registrarte o reactivar: ${process.env.ENVIRONMENT == "DEV" ? "https://dev.savemoneyy.com" : "https://www.savemoneyy.com"}
`
