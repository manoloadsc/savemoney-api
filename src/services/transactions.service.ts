...
class TransactionService {
  ...
  async getTransactions(
    ...
    const [entries, total] = await prisma.$transaction([
      prisma.transaction.findMany({
        ...
        ...(getAll ? { orderBy: { id: "desc" } } : { orderBy: { id: "desc" }, take: limit, skip: (page - 1) * limit }),
      }),
  ...
  async createTransaction(data: createTransactionDTO, userId: string) {
    ...
    if (data.recurring > 1) {
      notificatation = await notificationService.createTransactionNotification(userId, {
        transactionId: transaction.id, description, value: value, referenceDate: new Date(nextReferenceDate), count: recurring, period: recurringType, categoryId: transaction.categoryId, type: transaction.type
      });
    }
  ...
  async updateTransaction(data: updateTransactionDTO, userId: string, transactionId: number) {
    ...
    const updated = await prisma.parcels.updateMany({
      data : {
        value : value
      },
      where : { 
  ...
  async createNotificationParcel(notificationId: number) {
    ...
    let remaing = notification.notificationTimes! - notification.recurrenceCount!
    ...
    if (!haveTransaction) {
      let transaction = await this.createTransaction({
        ...
        firstDate: notification.nextNotificationDate?.toISOString()
      }, notification.userId)
