...
import { userDateTime2 } from "lib/date.js";
import { Interval } from "lib/prisma.js";
import { z } from "zod";

export const createNotificationValidation = (userTimezone : string) => z.object({
  period: z.enum(Object.values(Interval) as [string, ...string[]]),
  referenceDate: z.string().transform((value, ctx) => { 
    try {
      const { databaseDate } = userDateTime2(value, userTimezone)
...
export type CreateNotificationDTO = z.infer<ReturnType<typeof createNotificationValidation>>
...
export const updateNotificationValidation = z.object({
  value: z.number().transform(value => Decimal(value)).optional(),
...
export const updateNotificationValidationGpt = ( userTimezone : string ) => z.object({
  id: z.number(),
  value: z.number().transform(value => Decimal(value)).optional(),
  description: z.string().min(5, { message: "A descrição deve ter pelo menos 5 caracteres" }).optional(),
  type: z.enum(["GASTO", "GANHO"]).optional(),
...
export type updatedNotificationGptDTO = z.infer<ReturnType<typeof updateNotificationValidationGpt>>;
