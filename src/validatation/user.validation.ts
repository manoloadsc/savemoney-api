...
export const parseLogin = z.object({
  email: z.string(),
  password: z.string(),
  timezone: z
    .string()
    .refine((timezone) => validateTimezone(timezone), {
...
export const userCheckoutValidation = z.object({
  name: z.string(),
  phone_number: phoneSchema,
  email: z.string().email("Não é um email válido"),
});
...
export const changePassowordToken = z.object({
  id: z.string(),
  email: z.string(),
  type: z.string(),
});
