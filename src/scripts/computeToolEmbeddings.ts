...
const POSITIVES: Partial<Record<ToolFunctionNames, string[]>> = {
  ...
  delete_notification: ["eliminar recordatorio 789", "borrar notificación 321"],
  delete_parcel: ["eliminar cuota 012", "borrar cuota 345"],
...
const NEGATIVES: Partial<Record<ToolFunctionNames, string[]>> = {
  ...
  add_finance: ["listar mis movimientos", "borrar transacción"],
  list_analysis: ["borrar transacción", "añadir gasto", "editar transacción"],
  update_transaction: [
    "listar mis movimientos",
    "borrar transacción",
    "añadir gasto",
  ...
  create_notification: ["crear una nueva transacción"],
};
...
async function computeToolEmbeddings(): Promise<void> {
  ...
  for (const tool of tools) {
    ...
    try {
      ...
      console.error(
        `Error al calcular embedding para ${tool.function.name}:`,
        err
      );
    }
  }
